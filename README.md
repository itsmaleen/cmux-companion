...
Open `ios/cmux.xcodeproj` in Xcode, select your iPhone, and run. The project
is generated by [XcodeGen](https://github.com/yonaskolb/XcodeGen) from
`ios/project.yml` (`cd ios && xcodegen generate` after adding files) and
depends on [SwiftTerm](https://github.com/migueldeicaza/SwiftTerm) via Swift
Package Manager, so from the command line build the **scheme**:

```bash
cd ios && xcodebuild -project cmux.xcodeproj -scheme cmux -sdk iphonesimulator26.5 -configuration Debug build
./scripts/run-tests.sh      # UIKit-free logic suites
./scripts/screenshots.sh    # renders recorded terminal frames in the simulator → ios/screenshots/*.png
```
...
When the runtime can stream a pane's rendered output (herdr can; cmux cannot
yet, see `plans/cmux-frames-proposal.md`), the focused card is a real terminal
emulator fed by the bridge's `surface.frame` push: colours, cursor and exact
layout, updated as the pane repaints instead of on a 3-second poll. The pane
is observed at its native size on the Mac (never resized), the font is fitted
to the card width, and a plain shell's scrollback is available by dragging
up. Surfaces without a frame stream fall back to the polled text card.

### Claude and opencode surfaces

A surface running Claude Code or opencode shows the **conversation itself** —
...
Pull past the top of a focused text card, or tap the history button on a
live-terminal card, to read the same conversation full-screen at a larger
size.
...
show each other's conversation. opencode keeps its sessions in SQLite; herdr
reports the session id directly, and on cmux the bridge proves a surface runs
opencode by its tty and matches the session by title and working directory.
See [shared/protocol.md](shared/protocol.md).
